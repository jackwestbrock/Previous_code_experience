...
class Grade {
 public:
  ...
  //  * Precondition: both parameters are > -1; total points must be greater
  //                  than 0. You may assert.
  ...
  //    total points earned.
  //  * Does not modify calling instance; const method.
  double ToDouble(unsigned int precision = 3) const;
  ...
  //    range at which a + is assigned. For example, when plus threshold is 75,
  //    then:
  //      A+ : grade percent >= 97.5
  //      ...
  //      D+ : grade percent >= 67.5
  //    If plus threshold is a zero, there is no plus. THE DEFAULT VALUE OF
  //    PLUS THRESHOLD PARAMETER IS 0.
  ...
  //      F : otherwise
  //  * Does not modify calling instance; const method.
  const std::string ToLetter(unsigned int threshold = 0) const;
  ...
  //  * Does not modify calling instance; const method.
  bool Equals(const Grade& right_hand_side) const;
  ...
  //  * Does not modify calling instance; const method.
  const Grade DividedBy(const Grade& right_hand_side) const;
  ...
  //    The equation is calculated using fractional arithmetic.
  //  * Does not modify calling instance; const method.
  const Grade Plus(const Grade& right_hand_side) const;
